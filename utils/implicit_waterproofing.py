...
def create_grid_points_from_bounds_indexed(minimun, maximum, res):
    x = np.linspace(minimun, maximum, res)
    X, Y, Z = np.meshgrid(x, x, x, indexing='ij')
    X = X.reshape((np.prod(X.shape),))
    Y = Y.reshape((np.prod(Y.shape),))
    ...
    return points_list, indices

def create_grid_points_from_all_bounds(res, x_min, x_max, y_min, y_max, z_min, z_max):
